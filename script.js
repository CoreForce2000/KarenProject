...
function purchase(imageSrc, priceVal) {
    ...
    changeBudgetBy(priceVal.slice(0,-1))

    addItemToCart(imageSrc, priceVal)
    hideImage()
}

function changeBudgetBy(price) {

    budget = document.getElementById("totalBudget").innerHTML
    budget -= price 
...
function removeItemFromCart(cartItem, price, id) {
    budget = document.getElementById("totalBudget").innerHTML
    budget = parseInt(budget) + parseInt(price)
    document.getElementById("totalBudget").innerHTML = budget
